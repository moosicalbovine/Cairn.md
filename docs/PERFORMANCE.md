...
Each run uses isolated app metadata and never opens the user's library. The harness:
...
Forty startup samples make the p95 estimate less sensitive to one-off hosted-runner provisioning noise while retaining every valid sample. The raw report also records aggregate working set for diagnosis. A non-zero exit means at least one enforced threshold failed.
