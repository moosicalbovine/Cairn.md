...
- V5. **Windows desktop flows:** `npm run test:e2e` passes F1-F4, AE1-AE6, keyboard, accessibility, offline, and external-change scenarios through Tauri WebDriver on Windows.
...
- V8. **Installer validation:** `npm run release:validate` builds and smoke-tests the NSIS installer in clean Windows environments with WebView2 present and absent. The missing-runtime case may use the embedded bootstrapper during installation; the installed core workflow must then work offline.
