...
Cairn.md `v0.1.1` is an early prerelease, and development is moving toward the `v0.2.0` organization and search scope. Contributions are most useful when they improve implementation quality, tests, requirements, accessibility, workflows, or documentation.
...
drives its W3C protocol directly, uses isolated temporary library and app-data
directories, and clears its dedicated test-only WebView2 profile around every
run. It never opens the user's configured Cairn.md library or production profile.
